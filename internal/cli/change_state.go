...
func deriveChangeStateDetailed(rootPath string, node changeNode, outputCommand changeGitOutput) (string, []string) {
	...
	if node.TargetRelease != "" {
		...
		if pinErr != nil {
			...
			ok, receiptErr, clean, evalErr, receiptWarn := evaluateVerifiedRungAtCommit(rootPath, node, evidenceGit)
			if evalErr != "" {
...
func evaluateVerifiedRungAtCommit(rootPath string, node changeNode, outputCommand changeGitOutput) (ok bool, receiptErr error, clean bool, evalErr string, receiptWarn string) {
	if outputCommand == nil {
		outputCommand = changeEvidenceGitOutput
	}
	...
	verdict, rErr := changeReceiptStatus(rootPath, headNode.Folder, headNode, outputCommand)
	receiptErr = rErr
	ok = verdict.OK
	if receiptErr == nil && !verdict.OK && verdict.Reason != changeReceiptMissing && verdict.Reason != changeReceiptOK {
		receiptWarn = "receipt evaluation failed: " + verdict.Cause()
	}
	return ok, receiptErr, clean, "", receiptWarn
}
...
func changeStructurallyCleanForState(rootPath string, node changeNode, outputCommand changeGitOutput) (bool, string) {
	...
	_, _, clean, evalErr, _ := evaluateVerifiedRungAtCommit(rootPath, node, outputCommand)
