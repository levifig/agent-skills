...
- Development builds stamp their source commit into the native binary with `go build -buildvcs=true` and report `<version>+g<short-sha>`, adding `.dirty` when the working tree had uncommitted changes at compile time; the build script links the same identity via `-X` for toolchains that do not stamp linked worktrees. The `bin/.loaf-dev-commit` sidecar, which could name a commit whose source was not what got compiled, is removed along with the byte-for-byte rebuild assertion in `verify:go-artifacts`; root `bin/` is no longer tracked and is regenerated by `npm run build:go`.
