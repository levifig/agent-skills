...
- [x] H1 dogfood — a user-invoked checkpoint, because agents never initiate a pitch: project-scale run on a real new greenfield project (not a scratch exercise). The candidate plugin was loaded from this repository's own working tree at a recorded commit — the user's Claude config targets `plugins/loaf` in this repo directly, so no `--plugin-dir` was needed and the candidate is pinned by construction. The produced `docs/BRIEF.md` stays in the user's project and is not copied here. Capture the run (scale, candidate commit, outcome, user reaction) and the revision list it produced as `research/pitch-interview-dogfood.md` in this change folder (no project names or BRIEF body content), then revise the guide and SKILL.md accordingly (closing ceremonies at both scales; interview depth: destination pinning, open-question specifiability, HITL/AFK tags, scenario stress-testing, challenge stance).
