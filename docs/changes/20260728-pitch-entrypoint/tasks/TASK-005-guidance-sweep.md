...
- [x] Run the full verification contract V1–V7 as declared in `shape.md`: `go test ./... -count=1`, `npm run build`, the per-hook `loaf check` chain (V3 enumerates the registry), the five-projection presence check, the explore-demotion grep, `loaf change check docs/changes/20260728-pitch-entrypoint --require-executable`, and the human-entry-guard greps.
