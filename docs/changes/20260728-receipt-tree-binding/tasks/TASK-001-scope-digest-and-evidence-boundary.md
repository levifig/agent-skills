...
- [ ] Implement `scopeDigest` per the pinned serialization; entries from `ls-tree` only, never the filesystem.
- [ ] Export the evidence-boundary constant with the three mask groups; settle the glob grammar (component-anchored vs prefix) and record the choice here for the ADR.
- [ ] Emit `scope_sections`: per-top-level-directory sub-digests from the same filtered, sorted entry stream.
- [ ] Tests under `TestChangeScopeDigest`: identical trees digest identically regardless of `core.quotePath`; sort independence from traversal order; mode change (100644→100755) changes the digest; excluded paths never participate; a receipts-only or reports-only commit leaves the digest unchanged; case-sensitive mask matching.
