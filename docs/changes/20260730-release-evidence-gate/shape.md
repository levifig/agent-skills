...
Provenance for 5–7: review rounds 1–3 (Codex GPT 5.6 Sol), all findings accepted; R2 replaced R1's resume mechanism wholesale, R3 bound admission to verified content.

5. **Resume admission is verify-then-restore, bound to provable prepare output.** At a gate refusal the run restores only the changelog it wrote, leaving version files at the candidate so the smoke runners hash candidate-versioned artifacts. On rerun: version files are admitted only when byte-equal to the candidate rendering derived from HEAD; dirty changelog always gets the classic clean-worktree refusal (operator curation is never restored or admitted); only build-owned generated outputs are restore-and-regenerated; evidence files are admitted only when referenced by the HEAD or worktree registry; deleted tracked files and untracked files under generated roots refuse by name. No persisted resume state, no flags. Dry-run and post-merge never restore.
6. **Receipt-only post-merge recovery, parent-anchored and typed.** Guardrails recognize exactly one repair commit atop the release commit whose NUL-delimited, rename-rejecting diff touches only installed-smoke receipt sources named by the parent commit's registry — the registry itself unchanged; the tag lands on repaired HEAD. A registry or non-receipt change means redoing the release PR, not a repair.
7. **Registry probe is symlink-hostile, component-wise.** Every path component from the repository root is `Lstat`-checked (no symlinked directories, regular-file leaf); a symlinked component is present-but-unusable and refuses, never silently absent.

## Planning Contract

### Placement

`--pre-merge` and the direct door share one executor, `runReleaseApply` (`internal/cli/release_dry_run.go`), so a single insertion covers both: the gate joins the existing post-rebuild refusals between the artifact-command loop and the `git add -A` staging step, as a third sibling to the virtualenv-leak and `docs/changes`-mutation checks — inspect the rebuilt tree, refuse before staging; on refusal the run restores the changelog it wrote and leaves version files at the candidate plus the rebuilt artifacts in place, so the smoke runners can hash candidate-versioned artifacts before the rerun (Decision 5). On `--post-merge` no rebuild occurs; the gate becomes **guardrail 9** in `checkReleasePostMergeGuardrails` (`internal/cli/release_post_merge.go`, after the HEAD-not-tagged check), inheriting the numbered `guardrail N failed` reporting and the injectable command runner, validating the merged tree as-is. Dry-run never rebuilds and stays gate-free.

The gate is the **first production caller** of `LoadTargetCapabilityEvidence` (`internal/cli/target_capability_contract.go:262`) — one argument, the full JSON path; it derives its anchor root internally. Presence probe: a component-wise `os.Lstat` walk from the repository root to `TargetCapabilityEvidenceRecordPath` (no symlinked directories, regular-file leaf, per Decision 7); truly absent → silent no-op, symlinked or irregular → refuse. The root is the **release root** (the tree being released), deliberately not `resolveSourceCheckoutRoot` — the gate certifies the tree that will be committed and tagged.
