...
Provenance for 5–7: review rounds 1 and 2 (Codex GPT 5.6 Sol), all findings accepted; R2 replaced R1's resume mechanism wholesale.

5. **Resume is restore-and-regenerate, not tolerance.** After a gate refusal, rerunning restores prepared-shaped tracked dirt (version files, changelog, generated outputs) from HEAD — keeping only the evidence class (registry + receipts), which is the operator's intentional input — then regenerates everything through the normal prepare. Untracked files under generated roots refuse by name. No persisted resume state, no `--continue` flag, no tolerant writes; hand edits are clobbered, never committed. Dry-run and post-merge never restore.
6. **Evidence-repair-aware post-merge recovery, parent-anchored.** Guardrails recognize exactly one receipt-only repair commit atop the release commit, with allowed paths derived from the parent commit's registry and the registry itself unchanged; the tag lands on repaired HEAD. Recovery copy prescribes re-record → single evidence-only commit → rerun; a registry change means redoing the release PR, not a repair.
