...
`TestSchemaDocumentationMirrorsExecutableMigration` keeps the `0001`–`0009` and `0011` SQL mirrors exact and checks that the DBML and Mermaid views include every executable table, column, and Mermaid relationship label. It validates against the **auto-applied baseline** (`SchemaMigrations()`, migrations `0001`–`0009` and `0011`); the `operational-state.*` views therefore model that baseline, with the journal-first end state documented as an annotated delta in each file's header.
...
Migration 0011 deliberately omits foreign keys from `journal_origins` and `journal_deferrals` to `journal_entries` and `sparks`: the explicit 0010 transformation may rebuild `journal_entries` after 0011 has applied, so optional provenance is audited by the owning workflows instead of creating a migration-order dependency.
