...
**The commit is stamped into the binary by the toolchain.** `loafdev build-go` (formerly `build-go.mjs`) passes `-buildvcs=true`, so `go build` records `vcs.revision` and `vcs.modified` in the binary's build info and `cmd/loaf/main.go` reads them through `runtime/debug.ReadBuildInfo`. The identity is mechanical — it describes the bytes that were compiled — and needs no file beside the binary that could be written from one HEAD while the source on disk said another, which is exactly how `bin/.loaf-dev-commit` produced a stale label on 2026-09-05. That file is gone, and so is the byte-for-byte rebuild assertion in `verify-go-artifacts.mjs`, whose only purpose was keeping tracked binaries identical across commits: root `bin/` is no longer tracked, so nothing needs that property. As of 2026-09-06 the Claude Code plugin ships only a shell shim at `plugins/loaf/bin/loaf` that resolves an installed `loaf`, so no native binary is tracked anywhere. The toolchain pin matters: `go1.26.6` wrote no stamp inside a linked worktree (a `.git` file), and worktrees are where Loaf's issue branches live, so `go.mod` pins `go1.27.1`, the first toolchain observed to stamp them. A malformed stamp is treated as absent. Staging still applies: `loafdev build-go` compiles requested targets into `bin/native/.staging/` and publishes the set only after every requested target compiles. A binary compiled outside any checkout reports the package version with the dev-channel suffix and no commit.

**The last successful dev build owns Loaf's user-local launcher pointer.** After every requested target compiles, `loafdev build-go` retargets `$XDG_DATA_HOME/loaf/current-dev-launcher` at the checkout's launcher. `~/.local/bin/loaf` is created only when that name is absent, as a symlink to the pointer. Release-metadata builds never claim it, `LOAF_DEV_LINK=0` is the explicit opt-out, and the build never overwrites, unlinks, or renames a real file, directory, or any other symlink at the PATH name — including a leftover checkout symlink from the previous scheme. Activation is best-effort: I/O or permission failures warn and leave the native build successful. This makes the active CLI follow whichever worktree was most recently built once the PATH name points at the pointer, without making the main checkout a privileged indirection point.
...
Adopted on 2026-09-05 once root `bin/` left version control. With the plugin reduced to a shim on 2026-09-06, no committed binary remains to hold byte-stable, so build-varying bytes cost nothing; `-buildvcs=true` supplies the commit and dirty flag with no custom linker flag.
...
- 2026-09-06 — The Claude Code plugin ships a shell shim instead of native binaries; the two-fact dev-identity rule stays because Homebrew, release archives, and npm still ship prebuilt binaries without release metadata.
