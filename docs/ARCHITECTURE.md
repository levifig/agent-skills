...
Linear is the only provider mapping in this slice. Its capability file maps each common operation to one externally pinned native semantic, maximum fidelity, and phased `before`, `execute`, and `after` requirements, while availability remains runtime-discovered. This proves required current-state and readback semantics without claiming that every harness connection exposes every operation or that semantic capability identifiers are universal tool names. No GitHub or Jira placeholder mapping exists. `flow-contract.json` makes main-agent execution through the selected provider skill primary. The optional project-manager profile points to that same machine behavior source and has no independent operation or retry policy; if a harness cannot enforce its connector-only boundary, execution remains with the main agent.

The root-module `internal/vnextflowcontract` package is test-only and outside `vnext/`. Its standard-library validator accepts an `fs.FS`, rejects duplicate JSON object keys recursively before strict typed decoding, and compares every content file and directory to an external recursive allowlist while rejecting symlinks. It also checks exact ceremony and Linear mappings, frontmatter, references, links, template markers, authority, and profile derivation through deterministic negative fixtures. A bounded phrase corpus guards known local-authority and provider-transport regressions in allowed Markdown and JSON, but it is defense in depth rather than a claim of arbitrary prose understanding. `go test ./internal/vnextflowcontract ./vnext/... -count=1 -run 'Flow|Template|TrackerSkill'` is the canonical Flow gate.
...
Permanence taxonomy: **ledger** (journal, decisions, wraps), **notebook** (sparks, ideas), **scratchpad** (effort-scoped agent messages). LOAF-87 ships `loaf scratchpad append|read|list|claim|release` with closed fact kinds. LOAF-88 ships server SSE/long-poll fanout on `loaf serve` (bucket backends degrade to poll). LOAF-89 ships logical close (`scratchpad_close` fact hides channels) and admin-only `loaf scratchpad prune` for relay blob deletion.
