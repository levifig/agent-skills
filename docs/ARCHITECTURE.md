...
The active test harness is Go. `make test` runs `go test ./...`, and `make typecheck` compiles all Go packages with `go test ./... -run=^$`. Build, release, packaging, and tag classification live in `internal/devtool`, driven by `go run ./cmd/loafdev` through the Makefile; npm and the Node launcher are gone, and `bin/loaf` is the native binary itself.
...
A dev build's commit identity (`<package-version>+g<short-sha>`, ADR-026) follows the same rule. `loafdev build-go` compiles with `-buildvcs=true`, so the toolchain stamps `vcs.revision` and `vcs.modified` into the native binary and `cmd/loaf/main.go` reads them back through `runtime/debug.ReadBuildInfo`; a build from a tree with uncommitted changes reports `.dirty` after the commit. `go.mod` pins `go1.27.1` because `go1.26.6` wrote no stamp inside a linked worktree, where Loaf's issue branches live. Because the identity is part of the compiled bytes it cannot drift from them, and it also means two builds of identical source from different commits differ, which is why no native binary is tracked any more: root `bin/` is a build output regenerated by `make build-go`, and the Claude Code plugin ships only a shell shim at `plugins/loaf/bin/loaf` that resolves an installed `loaf` (`LOAF_BIN`, `PATH`, then the usual install directories) at run time. `loafdev build-go` still compiles requested targets into `bin/native/.staging/` and publishes them only after every requested target compiles, so a partial rebuild cannot leave a mixed set. After a successful non-release build, the same script retargets `$XDG_DATA_HOME/loaf/current-dev-launcher` and creates `~/.local/bin/loaf` only when that name is absent, as a symlink to the pointer; `LOAF_DEV_LINK=0` disables this behavior, an existing real file, directory, or any other symlink is never overwritten, and activation failure never fails the native build.
