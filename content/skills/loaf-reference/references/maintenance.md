...
The managed `<!-- loaf:managed:start … -->` section is fingerprint-identified (`sha256=` of the body). `loaf doctor`'s `fenced-content` check compares the actual body to the constant this binary generates: tamper (stored fingerprint disagrees with the body) is reported first and must be reconciled by hand — `loaf upgrade` will refuse that state; content drift (intact section whose body differs from the generated constant) is remedied with `loaf upgrade`. A pending one-time stamp-strip transition from a legacy `v… sha256=…` header is not itself a warning.
