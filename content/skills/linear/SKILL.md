...
name: linear
description: >-
  Manages Linear issues, projects, cycles, documents, and team workflows through whichever Linear MCP server the current harness exposes. Use when the user asks to triage, plan, create, update, audit, or report on Linear work. Produces verified mutations or read-only summaries with gaps and blockers. In Loaf projects, preserves loaf issue authority and synchronization boundaries. Not for configuring or authenticating MCP servers.
---
