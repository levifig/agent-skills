...
3. Select the next work item:
   ...
   - Else, if multiple unblocked `unstarted` sub-issues exist, use
     `built-in chat clarification` to let the user choose: pick one, or delegate N in
...
Use the **background agent** with appropriate `agent_type`:
...
1. **Strict delegation** -- ALL implementation via background agent
...
9. [ ] Break down work using task list or chat checklist
