...
Series-prep (bootstrap Finalization) turns the project BRIEF's scoped concepts into backlog issues. This section resolves when a concept earns a row versus staying a BRIEF line or spark. Full procedure: bootstrap SKILL.md Finalization → Series-Prep.

### Earns its own backlog issue when both are true

1. **Independently rideable** — the concept names a complete operator journey that could land as a meaningful release unit without waiting on the other scoped concepts to become useful. A storage, backend, protocol, API, UI, or verification layer alone does not pass. Sharing an advisory bucket is fine (buckets are labels, never bindings); hard entanglement is not.
...
- It is a foundation or component layer whose value appears only after a future issue; keep it inside the immediate consuming journey.
...
- Challenge under-splitting: "Is this independently rideable, or are we smuggling two products into one issue?"
...
Each minted issue body must name its problem without reference to the bootstrap session or to `docs/BRIEF.md`. If a draft says "as discussed" or "per the project brief," rewrite until the problem stands alone.
