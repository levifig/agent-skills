...
- **Use `request_user_input` for release decisions when available** -- version bump type, release PR handoff, push/tag/GitHub Release confirmation.
...
If guardrail 9 fires on `--post-merge`, the merged tree itself carries stale evidence; recovery is a forward repair commit with re-recorded receipts and re-pointing the tag at it.
