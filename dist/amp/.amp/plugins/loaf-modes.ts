...
// Restored from the user's disabled Loaf 0.3.1 mode definitions.
// Kept separate from Loaf's managed hook plugin so upgrades preserve it.
...
const ORACLE_INSTRUCTIONS = `You are a pinned second-opinion specialist (GPT-6 Astra, high reasoning). You do not implement and you do not own the user's thread.
...
const SHARED_ORCH_OVERLAY = "\n# Loaf orchestration overlay\n\nThese rules supersede the Oracle and Subagents sections above when they conflict.\n\nYou are the main orchestrator in this thread. Investigate, plan, decide, and report here. Do not implement substantial code in this thread, and do not perform the independent review yourself.\n\n## Delegation (mandatory defaults)\n\n- Implementation: call `delegate_implementation` for bounded coding work. That tool is pinned to Grok 4.6 with Fast. Give it a complete brief: outcome, paths, constraints, and verification commands. If the tool fails, report the failure and stop — do not silently implement here and do not switch models.\n- Review: call `delegate_review` after implementation, or when the user asks for a code review. Prepare a complete diff yourself (staged, unstaged, and relevant untracked contents). That tool is pinned to GPT-5.6 Luna at max. If it fails, report the failure — do not substitute your own review as the review of record.\n- Stay in this thread for questions, planning, diagnosis, and orchestration. Tiny one-line fixes may stay here only when asking the implementation agent would cost more than the change; when in doubt, delegate.\n\nDo not spawn the implementation or review agents by creating a mode-picker thread. They are not selectable modes. Use the tools above. Do not use `create_thread` / Task to reimplement those workers.\n\n## Oracle — maximize usefulness\n\nAmp does not call Oracle automatically. You must invoke it. Prefer `consult_oracle` over the built-in `oracle` tool: it is pinned to GPT-6 Astra at high reasoning and will not silently substitute another model. Use built-in `oracle` only if `consult_oracle` is unavailable.\n\nInvestigate first. Then call Oracle in the same turn when any of these are true:\n\n- Two or more plausible designs, APIs, or root causes remain after you have read the relevant code\n- The change is cross-cutting, concurrent, security-sensitive, or hard to reverse (schema, protocol, public API, data migration, authz)\n- You have a concrete plan and need a check for a simpler approach or a missed invariant before coding\n- A risky implementation is about to land and an invariant is still unresolved (review covers the diff; Oracle judges the invariant)\n- The user asks whether a design is right, whether there is a better solution, or to be thorough\n\nDo not call Oracle for typos, local mechanical edits, questions answered by a file you have not read yet, or to rubber-stamp a decision you have already made. Do not call Finder/Librarian work \"Oracle work.\"\n\nWhen you call `consult_oracle`, ask one unresolved question. Include intended behavior, constraints, candidate theories, files, and what you already ruled out. Treat the answer as advisory; you still own the decision. Name that you consulted Oracle and what changed because of it.\n\n## Librarian and other specialists\n\nKeep Librarian, Finder, Task, web tools, and skills. Librarian is for code you cannot fully read locally. Finder is for local conceptual search. Task is only for independently specifiable work that is not implementation or review.\n" as string;
const MEDIUM_OVERLAY = SHARED_ORCH_OVERLAY + "\nThis mode is the everyday Loaf orchestrator (Medium-shaped): GPT-6 Astra at medium reasoning. Prefer it for normal messy work. Do not inflate the task into Ultra-scale exploration. Use Finder and parallel Task workers freely for investigation, the way Medium does. Delegate implementation and review as above. Use Oracle on the criteria above rather than saving it for emergencies — a second opinion before implementation is cheaper than a wrong implementation.\n";
const ULTRA_OVERLAY = SHARED_ORCH_OVERLAY + "\nThis mode is the hard-problem Loaf orchestrator (Ultra prompt and tool set; High-like model stack): GPT-6 Astra at xhigh reasoning, with `consult_oracle` pinned to GPT-6 Astra at high. Default to Oracle before delegating implementation unless the change is local and unambiguous. Bias toward Oracle on architecture forks, concurrency, and invariants. Still delegate implementation and review as above. Do not implement the Ultra-scale change in this thread yourself.\n";
...
type PinnedAgentTextBlock = { type: 'text'; text: string };

function isPinnedAgentTextBlock(block: unknown): block is PinnedAgentTextBlock {
...
export function extractPinnedAgentText(content: unknown): string {
  ...
  for (const block of content) {
    if (!isPinnedAgentTextBlock(block)) {
      continue;
    }
...
async function runPinnedAgent(
  ...
  try {
    ...
    const text = extractPinnedAgentText(reply.content);
...
function registerDelegatedAgents(amp: PluginAPI): void {
  ...
  assertDelegateCompatibility('implementation', IMPLEMENTATION_MODEL, IMPLEMENTATION_TOOLS);
  assertDelegateCompatibility('review', REVIEW_MODEL, REVIEW_TOOLS);
  assertDelegateCompatibility('oracle', ORACLE_MODEL, ORACLE_TOOLS);
  assertDelegateCompatibility('loaf-medium', ORCHESTRATOR_MODEL, ORCHESTRATOR_TOOLS);
  assertDelegateCompatibility('loaf-ultra', ORCHESTRATOR_MODEL, ORCHESTRATOR_TOOLS);

  const implementationAgent = createPinnedAgent(amp, 'implementation', IMPLEMENTATION_MODEL, IMPLEMENTATION_TOOLS, {
    name: 'loaf-implementation-agent',
    model: 'xai/grok-4.6',
    features: ['fast'],
    instructions: IMPLEMENT_INSTRUCTIONS,
    tools: IMPLEMENTATION_TOOLS,
    display: { label: 'Loaf Implementation', color: '#0ea5e9' },
  });

  const reviewAgent = createPinnedAgent(amp, 'review', REVIEW_MODEL, REVIEW_TOOLS, {
    name: 'loaf-review-agent',
    model: 'openai/gpt-5.6-luna',
    reasoningEffort: 'max',
    instructions: REVIEW_INSTRUCTIONS,
    tools: REVIEW_TOOLS,
    display: { label: 'Loaf Review', color: '#8b5cf6' },
  });

  const oracleAgent = createPinnedAgent(amp, 'oracle', ORACLE_MODEL, ORACLE_TOOLS, {
    name: 'loaf-oracle-agent',
    model: 'openai/gpt-6-astra',
    reasoningEffort: 'high',
    instructions: ORACLE_INSTRUCTIONS,
    tools: ORACLE_TOOLS,
    display: { label: 'Loaf Oracle', color: '#f59e0b' },
  });

  const mediumOrch = createPinnedAgent(amp, 'loaf-medium', ORCHESTRATOR_MODEL, ORCHESTRATOR_TOOLS, {
    name: 'loaf-medium',
    model: 'openai/gpt-6-astra',
  ...
  const ultraOrch = createPinnedAgent(amp, 'loaf-ultra', ORCHESTRATOR_MODEL, ORCHESTRATOR_TOOLS, {
    name: 'loaf-ultra',
    model: 'openai/gpt-6-astra',
  ...
  registerPinnedAgentMode(amp, 'loaf-medium', ORCHESTRATOR_MODEL, ORCHESTRATOR_TOOLS, {
    key: 'loaf-medium',
    label: 'Loaf Medium',
    description: 'Loaf Medium orchestrator: GPT-6 Astra medium plans and decides; Grok 4.6 with Fast implements; Luna max reviews; pinned Astra-high oracle.',
    color: '#eab308',
    agent: mediumOrch.definition,
  });

  registerPinnedAgentMode(amp, 'loaf-ultra', ORCHESTRATOR_MODEL, ORCHESTRATOR_TOOLS, {
    key: 'loaf-ultra',
    label: 'Loaf Ultra',
    description: 'Loaf Ultra orchestrator: Ultra harness, GPT-6 Astra xhigh plans and decides; Grok implements; Luna reviews; pinned Astra-high oracle.',
    color: '#ea580c',
    agent: ultraOrch.definition,
  });

  registerPinnedTool(amp, 'implementation', IMPLEMENTATION_MODEL, IMPLEMENTATION_TOOLS, {
    name: 'delegate_implementation',
    title: 'Delegate implementation',
    description:
      'REQUIRED default for implementation in Loaf Medium and Loaf Ultra. Delegate one bounded coding issue to Grok 4.6 with Fast. Fail closed: do not implement in the orchestrator thread if this tool errors. Provide the complete issue contract, a Loaf-started isolated worktree or appropriate runner path, constraints, and verification commands. Built-in Amp medium cannot be rewritten; use Loaf Medium. After invocation, Grok 4.6 with Fast is exact with a finite local coding allowlist. Unavailable delegates fail closed with no silent fallback and no local fallback.',
    inputSchema: {
      type: 'object',
      properties: {
        workdir: {
    ...
    async execute(input, ctx) {
      ...
      const result = await runPinnedAgent(implementationAgent, 'implementation', IMPLEMENTATION_MODEL, IMPLEMENTATION_TOOLS, prompt, ctx);
      return `Implementation thread: https://ampcode.com/threads/${result.threadID}\n\n${result.text}`;
    },
  });

  registerPinnedTool(amp, 'review', REVIEW_MODEL, REVIEW_TOOLS, {
    name: 'delegate_review',
    title: 'Delegate review',
    description:
      'REQUIRED default for review in Loaf Medium and Loaf Ultra. Delegate an independent read-only review to GPT-5.6 Luna (max). Fail closed: do not substitute an orchestrator review if this tool errors. Provide intent, issue criteria, a nonempty caller-prepared diff/snapshot, and relevant verification evidence. Loaf itself must not execute Git to prepare the snapshot. After invocation, Luna max is exact and read-only. Unavailable delegates fail closed with no silent fallback and no local fallback.',
    inputSchema: {
      type: 'object',
      properties: {
        workdir: {
        ...
        diff: {
          type: 'string',
          description:
            'Complete unstaged and staged diff prepared by the orchestrator. Include untracked file contents when relevant. Caller-prepared only; do not ask the review agent or Loaf to run Git.',
        },
        prompt: {
          type: 'string',
          description:
    ...
    async execute(input, ctx) {
      ...
      const result = await runPinnedAgent(reviewAgent, 'review', REVIEW_MODEL, REVIEW_TOOLS, prompt, ctx);
      return `Review thread: https://ampcode.com/threads/${result.threadID}\n\n${result.text}`;
    },
  });

  registerPinnedTool(amp, 'oracle', ORACLE_MODEL, ORACLE_TOOLS, {
    name: 'consult_oracle',
    title: 'Consult oracle',
    description:
      'Pinned second-opinion oracle: GPT-6 Astra at high reasoning, read-only. Prefer this over the built-in oracle tool. Call after you have read the relevant code, with one unresolved high-impact question plus evidence. Do not use for local search (Finder), external code (Librarian), implementation, or diff review. Fail closed with no silent fallback and no local fallback.',
    inputSchema: {
      type: 'object',
      properties: {
        question: {
    ...
    async execute(input, ctx) {
      ...
      const result = await runPinnedAgent(oracleAgent, 'oracle', ORACLE_MODEL, ORACLE_TOOLS, prompt, ctx);
...
export default function (amp: PluginAPI) {
  try {
    ...
    console.error(`[loaf-modes] registration failed. ${detail}`);
  }
}
