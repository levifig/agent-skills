...
3. Select the next work item:
   ...
   - Else, if multiple unblocked `unstarted` sub-issues exist, use
     `Amp UI input` to let the user choose: pick one, or delegate N in
...
Use the **Amp check/agent mode or new thread** with appropriate `agent_type`:
...
1. **Strict delegation** -- ALL implementation via Amp check/agent mode or new thread
...
9. [ ] Break down work using Amp thread checklist
