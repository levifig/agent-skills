...
|------------|----------------|------------------|-------------|
| Product / multi-audience concept | Specific roles required | Usually yes — researcher subtask agent | Full UVP sentence |
...
Use `prompt the user in chat` when available; otherwise one inline question per message with the same semantics. Never batch dimensions into a multi-field form — forms invite skimmed defaults and are the failure mode this ceremony exists to prevent.
...
When competitive landscape or external facts would change the brief and the human lacks them, pause the grill, delegate a researcher subtask agent, land evidence (change-scale: `research/` in the change folder; project-scale: links in Sources), then resume with a recommendation informed by the scan. Evidence supports framing; it does not become solution design.
