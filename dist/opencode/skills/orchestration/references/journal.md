...
only when the conversation holds synthesis worth saving — abandoned
...
See the `wrap` skill for the full checkpoint flow. Loose ends name issues
(`loaf issue frontier`, `loaf issue list --started`), not board leftovers.
...
Pass issue IDs and report references to background and delegated agents. The harness
...
3. Compare against `git status`, `git log`, `loaf issue frontier`, and `loaf issue list --started`.
