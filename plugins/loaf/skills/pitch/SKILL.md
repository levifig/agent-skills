...
Human problem-discovery ceremony. Authors a brief at the matching scale so `/loaf:shape` starts from a framed problem and bootstrap can consume a pitched project BRIEF.
...
3. **Problem-space only** — grill what, who, and why-valuable. Approach, architecture, decomposition, and verification design belong to `/loaf:shape`. A brief that reads like a pseudo-shape is a failure; rewrite before landing.
...
5. **Never write `shape.md`, seed `tasks/`, push, or open PRs** — pitch prepares commits and hands off; push and PR stay human. Never auto-run `/loaf:shape` or `/loaf:bootstrap`.
...
| Empty or minimal directory / greenfield intent | **Project** | `docs/BRIEF.md` with `source: pitch` → hand to `/loaf:bootstrap` |
...
|--------|--------|--------|--------|
| **Shape now** | Create the slug branch (`git switch -c <slug>`), stay there | Hand to `/loaf:shape` for in-place promotion — do not park-commit first | Stamp `target_release` when known |
...
- **Shape now** — slug branch + hand to `/loaf:shape`, which promotes the capture in place via ordinary `loaf change init <slug>` (no `--brief`).
...
8. **Execute the chosen landing:**
   ...
   - **Shape now:** `git switch -c <slug>` (from default unless already on a working branch the human prefers), ensure pre-landing guard would pass if they later park, hand to `/loaf:shape` with the folder path — shape promotes in place. Do not open a PR.
...
10. **Closing ceremony (required — never trail off).** After the landing is executed (or intake retained), announce completion with a full closing block:
    ...
    - **Restate the landing actually taken** and what it means next:
      - **Shape now** — you are on the slug branch; run `/loaf:shape` next to promote the capture in place and bound implementation. No park-commit was made.
      - **Park targeted** — the capture is a docs-only commit on the default branch with `target_release` stamped; it sits as a promise carrier for that cohort until `/loaf:shape` is invoked later.
