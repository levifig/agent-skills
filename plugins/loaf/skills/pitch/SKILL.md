...
3. **Hand to `/loaf:bootstrap`** — recommend it explicitly; do not auto-run. Bootstrap recognizes `source: pitch` as discovery-already-done and interviews for gaps only (series-prep is bootstrap's closing phase, not pitch's).
...
Report the folder or BRIEF path, landing choice, and next step (`/loaf:shape` or `/loaf:bootstrap`) in plain language.
